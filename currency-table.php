<?php
/*
Plugin Name: Currency table
Plugin URI: http://www.TakeReal.com
Description: Grab currency rates. Display table.
Author: Sergey Grachyov aka TakeReal
Author URI: http://www.TakeReal.com
Version: 0.0.1
...
$myjson = file_get_contents(CURRENCYTABLEJSONURL);
...
if (preg_match_all("/(\"([A-Z]{3})\"\:\"([0-9\.]+)\")/", $myjson, $matches)) {
  ...
  <tr>
    <td align="right">&nbsp;</td>
    <td><p>Please refer to readme file included in this download for full installation instructions</p></td>
  </tr>
  </table>
<?php if (1) { ?>
<style>
...
<?php
  ...
  //wp_cron();
  //print wp_next_scheduled ('currency_table_cron_event');
  //print '<br/>';
  //print time();
...
function widget_currency_table_init() {
  ...
  function widget_currency_table_display($pmcArgs) {
    ...
    $myHTML .= '<a href="http://www.fx-foreignexchange.com/currency_widget.php?value=1.00&from=USD&to=GBP&r=widget"  onClick="window.name=\'exchange_rates_todayNew\';window.open(this.href,\'converter\',\'toolbar=no,location=no,directories=no,status=no,menubar=no,width=660,height=880,resizable=yes,scrollbars=yes\');return false;" style="font-size: 10px;">';
